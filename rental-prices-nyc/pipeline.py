...
def create_dataset(dataset_name):
    ...
    try:
        original_ds = Dataset.get(dataset_name=dataset_name, dataset_tags=["latest"])
        Logger.current_logger().current_logger().report_text("Dataset %s already exists, creating the child one" % dataset_name)
        ds = Dataset.create(dataset_name=dataset_name, parent_datasets=[original_ds.id])
    except ValueError:
        original_ds = None
        Logger.current_logger().current_logger().report_text("Dataset %s does not exists, creating the first one" % dataset_name)
...
@PipelineDecorator.component(return_values=['dataset_id'], cache=True, task_type=Task.TaskTypes.data_processing)
def clean_data(dataset_id, dataset_name_cleaned, min_price, max_price):
    ...
    df = df[idx].copy()
    Logger.current_logger().current_logger().report_text("Dataset price outliers removal outside range: %s-%s", min_price, max_price)
    ...
    try:
        ...
        Logger.current_logger().current_logger().report_text("Dataset %s already exists, creating the child one" % dataset_name_cleaned)
        ds = Dataset.create(dataset_name=dataset_name_cleaned)
    except ValueError:
        original_ds = None
        Logger.current_logger().current_logger().report_text("Dataset %s does not exists, creating the first one" % dataset_name_cleaned)
        ds = Dataset.create(dataset_name=dataset_name_cleaned)
...
@PipelineDecorator.component(cache=True, task_type=Task.TaskTypes.data_processing)
def check_data(dataset_id, min_price, max_price):
    from clearml import Dataset, Logger
    import pandas as pd
    import numpy as np
    import glob
    ...
    Logger.current_logger().report_text("Test column names: column names expected: %s",
                    list(expected_colums))
    Logger.current_logger().report_text("Test column names: column names in dataset: %s",
                    list(these_columns))
    assert list(expected_colums) == list(these_columns)

    known_names = ["Bronx", "Brooklyn", "Manhattan", "Queens", "Staten Island"]
    neigh = set(df['neighbourhood_group'].unique())
    Logger.current_logger().report_text("Test column names: neighbourhood group expected: %s",
                    set(known_names))
    Logger.current_logger().report_text("Test column names: neighbourhood group in dataset: %s",
                    set(neigh))
    assert set(known_names) == set(neigh)

    idx = df['longitude'].between(-74.25, - 73.50) & df['latitude'].between(40.5, 41.2)
    Logger.current_logger().report_text("Test proper boundaries: unexpected items are %s", np.sum(~idx))
    assert np.sum(~idx) == 0

    Logger.current_logger().report_text("Test row count: items are %s", df.shape[0])
    assert 15000 < df.shape[0] < 1000000

    items_ok = df['price'].between(min_price, max_price).shape[0]
    Logger.current_logger().report_text("Price range test, items in range are: %s", items_ok)
...
def executing_pipeline(dataset_name_raw, dataset_name_cleaned, min_price=10, max_price=350):
    
    raw_dataset_id = create_dataset(dataset_name_raw)
    cleaned_dataset_id = clean_data(raw_dataset_id, dataset_name_cleaned, min_price, max_price)
